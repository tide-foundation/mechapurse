...
export default function RecentTransactions({ transactions, isTransactionsLoading }: RecentTransactionsProps) {
  return (
    <div className="app-transactions-container h-[485px] flex flex-col overflow-hidden border border-gray-300 rounded-lg p-4">
      <h2 className="text-lg font-semibold text-[#0f172a] mb-3 text-center !pb-2">
        Recent Transactions
      </h2>
      {/* Scrolling container */}
        ...
        {isTransactionsLoading ? (
          ...
          <div className="!space-y-3">
            {transactions.map((tx) => (
              <div
                key={tx.tx_hash}
                className="app-transaction-row flex justify-between items-center bg-dark-card p-4 rounded-lg border border-dark-card-border"
              >
